...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  class_nomalizer = log_sum_exp_over_rows(class_input);
  log_class_prob = class_input - repmat(class_nomalizer,[size(class_input,1),1]);
  class_prob = exp(log_class_prob);
  
  num_cases = size(data.inputs,2);  %测试样本数量
  
  %%%%hid to output
  output_delta = (class_prob - data.targets);
  ret.hid_to_class = output_delta*hid_output';
  ret.hid_to_class = ret.hid_to_class./num_cases +wd_coefficient*model.hid_to_class;
  
  error_derivated = model.hid_to_class'*output_delta.*hid_output.*(1-hid_output);   %误差导数
  ret.input_to_hid = error_derivated*data.inputs';
